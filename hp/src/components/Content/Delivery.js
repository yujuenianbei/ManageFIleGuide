...
class Delivery extends Component {
    ...
    getGoodsResInfoData = (result) => {
        if (result.data.queryGoodsResInfoByUserName.length === 0) {
            ...
            this.props.changeOrderAddAddress(false);
            this.props.getOrderAddress(result.data.queryGoodsResInfoByUserName)
            this.setState({
    ...
    addNewAddress = () => {
        if (this.props.state.order.addAddressState) {
            ...
            this.props.changeOrderAddAddress(true);
            this.props.form.setFieldsValue({
                email: this.props.state.user.useremail,
            })
        }
        ...
        return (
            <div className={styles.delivery}>
                ...
                <div className={styles.deliveryContent}>
                    {this.props.state.user.loginState === 0 &&
                        <Fragment>
                            <Message type="warn">请先<Link to={'/account/login'}>登录</Link>后再提交订单</Message>
                        </Fragment>
                    }
                    {this.props.state.user.loginState === 1 &&
                        <Fragment>
                            <div className={styles.userInfo} ref={ref => { this.form = ref }}>
                                <div className={styles.leftContent + " " + styles.deliveryAddress}>
                                    ...
                                    {(this.props.state.order.orderAddress.length === 0 || this.props.state.order.addAddressState || this.props.state.user.loginState === 0) &&
                                        <Fragment>
                                            <DeliveryAddress
                                                form={this.props.form}
                                                loginState={this.props.state.user.loginState}
                                                loginGoodsResInfo={this.props.state.order.orderAddress} />
                                            <button className={styles.submitBtn + " " + styles.prime} onClick={this.submitNewAddress}>添加新收货地址</button>
                                        </Fragment>
                                    }
                                    {this.props.state.order.orderAddress.length === 0 && this.props.state.user.loginState === 0 &&
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        changeOrderAddAddress: (data) => { dispatch(Actions.orderAddAddress(data)); },
        // 购物车相关
