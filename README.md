...
本地虚拟机中（centos）
centos
docker run -p 8081:80 -p 8082:443 --name mynginx -v $PWD/:/usr/share/nginx/html -v $PWD/nginx/conf/nginx.conf:/etc/nginx/nginx.conf -v $PWD/nginx/conf.d/default.conf:/etc/nginx/conf.d/default.conf -v $PWD/nginx/conf.d:/etc/nginx/conf.d -v $PWD/logs:/var/log/nginx -d nginx
