...
function getFieldNamesByElasticType(fieldMap, types) {
  ...
  types.forEach(function (type) {
    if (typeof fieldMap[type] === 'object') {
      Object.keys(fieldMap[type]).forEach(function (fieldName) {
...
function getFieldNamesType(opts, types, typePrefix) {
  var addAll = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
  ...
  if (!typePrefix) {
    types.sort();
    typePrefix = types.map(function (t) {
      return (0, _graphqlCompose.upperFirst)(t);
    }).join('');
  }
  var name = (0, _utils.getTypeName)(`${typePrefix}Fields`, opts);
  var description = (0, _utils.desc)(`Avaliable fields from mapping.`);

  return (0, _utils.getOrSetType)(name, function () {
    if (!opts || !opts.fieldMap) {
      return 'String';
    }
    var values = getEnumValues(opts.fieldMap, types, addAll);
...
function getEnumValues(fieldMap, types) {
  var addAll = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
  ...
  getFieldNamesByElasticType(fieldMap, types).forEach(function (fieldName) {
    values[fieldName] = {
...
function getFieldConfigMap(opts, types, fc) {
  var addAll = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
  ...
  getFieldNamesByElasticType(opts.fieldMap, types).forEach(function (fieldName) {
    fcMap[fieldName] = fc;
  });
