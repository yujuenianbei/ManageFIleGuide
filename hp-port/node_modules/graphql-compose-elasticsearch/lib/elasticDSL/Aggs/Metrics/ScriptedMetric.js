'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.getScriptedMetricITC = getScriptedMetricITC;

var _graphqlCompose = require('graphql-compose');

var _utils = require('../../../utils');

var _Script = require('../../Commons/Script');

function getScriptedMetricITC() {
  var opts = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};

  var name = (0, _utils.getTypeName)('AggsScriptedMetric', opts);
  var description = (0, _utils.desc)(`
    A metric aggregation that executes using scripts to provide a metric output.
    [Documentation](https://www.elastic.co/guide/en/elasticsearch/reference/current/search-aggregations-metrics-scripted-metric-aggregation.html)
  `);

  return (0, _utils.getOrSetType)(name, function () {
    return _graphqlCompose.InputTypeComposer.create({
      name,
      description,
      fields: {
        init_script: function init_script() {
          return (0, _Script.getCommonsScriptITC)(opts);
        },
        map_script: function map_script() {
          return (0, _Script.getCommonsScriptITC)(opts);
        },
        combine_script: function combine_script() {
          return (0, _Script.getCommonsScriptITC)(opts);
        },
        reduce_script: function reduce_script() {
          return (0, _Script.getCommonsScriptITC)(opts);
        },
        params: `input ${(0, _utils.getTypeName)('AggsScriptedMetricParams', opts)} {
          field: String
          _agg: JSON!
        }`
      }
    });
  });
}