...
import { GraphQLString, GraphQLFloat, GraphQLBoolean, GraphQLObjectType, GraphQLEnumType, GraphQLNonNull } from 'graphql-compose/lib/graphql';
...
export default class ElasticApiParser {
  ...
  generateFieldMap() {
    ...
    return reorderKeys(fieldMap, ['cat', 'cluster', 'indices', 'ingest', 'nodes', 'snapshot', 'tasks', 'search']);
  }
  ...
  generateFieldConfig(methodName, methodArgs) {
    ...
    const { description, argsSettings, argsDescriptions, elasticMethod } = this.parsedSource[methodName];
    ...
    return {
      ...
      resolve: (source, args, context, info) => {
        ...
        if (Array.isArray(elasticMethod)) {
          return client[elasticMethod[0]][elasticMethod[1]](_extends({}, methodArgs, args));
        }
        return client[elasticMethod](_extends({}, methodArgs, args));
      }
    };
  }
