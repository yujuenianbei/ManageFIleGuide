...
export default function createSearchResolver(fieldMap, sourceTC, opts) {
  ...
  const searchITC = getSearchBodyITC({ prefix, fieldMap }).removeField(['size', 'from', '_source', 'explain', 'version']);
  ...
  return new Resolver({
    ...
    resolve: async rp => {
      ...
      if (args.opts) {
        args = _extends({}, args.opts, args, {
          body: _extends({}, args.opts.body, args.body)
        });
        delete args.opts;
      }
...
export function toDottedList(projection, prev) {
  let result = [];
